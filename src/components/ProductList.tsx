      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4 text-sm text-gray-600">
          ...
          <span className="flex items-center">
            ...
            Low Stock: {products.filter(p => p.stock_quantity < 10 && p.stock_quantity > 0).length}
          </span>
          <span className="flex items-center">
            ...
            <div key={product.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${stockStatus.bg} ${stockStatus.color}`}>
                  {product.stock_quantity < 10 && <AlertTriangle className="w-3 h-3 mr-1" />}
                  {stockStatus.label}
                </span>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <p className="font-medium">${product.wholesale_price.toFixed(2)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <p className="font-medium">${product.open_market_price.toFixed(2)}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <p className={`font-medium ${stockStatus.color}`}>{product.stock_quantity}</p>
                    </div>
                  </div>
                </div>
              </div>
